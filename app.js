(() => {
  ...
  const LEGACY_STORAGE_KEY = 'mandate_game_v4';
  const SLOTS_KEY = 'mandate_premiership_slots_v5';
  const ACTIVE_SLOT_KEY = 'mandate_active_premiership_v5';
  ...
  const initialState = () => ({
    version: 5,
    saveSlotId: null,
    careerCreatedAt: new Date().toISOString(),
    premiershipStartedDate: null,
    premiershipHistory: [],
    identityLocked: false,
    identityOrigin: 'created',
    personalVotes: {},
    electionHistory: [],
    localElections: { nextDate: null, current: null, history: [] },
    leadershipPressure: { active:false, weeks:0, score:0, lastWeek:0, calls:[] },
    leadershipContest: null,
    postElectionChoice: null,
    partyLeaders: {
      'Progressive Labour':'Eleanor Ward',
      'Conservative Union':'James Ashford',
      'Liberal Alliance':'Clara Bennett',
      'Reform Front':'Marcus Trent',
      'Green Movement':'Imogen Vale',
      'Civic Independents':'Noah Clarke'
    },
    partyColors: {...BASE_PARTY_COLORS},
    phase: 'setup',
    country: 'Avalon',
  ...
  let activeSlotId = localStorage.getItem(ACTIVE_SLOT_KEY) || null;
  let state = loadState();
  ...
  function makeSlotId(){return `pm-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;}
  function hydrateState(parsed){
    ...
    if(!Array.isArray(merged.premiershipHistory))merged.premiershipHistory=[];
    if(!merged.personalVotes)merged.personalVotes={};
    if(!Array.isArray(merged.electionHistory))merged.electionHistory=[];
    if(!merged.localElections)merged.localElections={nextDate:null,current:null,history:[]};
    if(!Array.isArray(merged.localElections.history))merged.localElections.history=[];
    if(!merged.leadershipPressure)merged.leadershipPressure={active:false,weeks:0,score:0,lastWeek:0,calls:[]};
    if(!Array.isArray(merged.leadershipPressure.calls))merged.leadershipPressure.calls=[];
    if(!merged.partyLeaders)merged.partyLeaders={...initialState().partyLeaders};
    if(!merged.partyColors){merged.partyColors={};Object.keys(merged.polls||merged.seats||{}).forEach(x=>merged.partyColors[x]=BASE_PARTY_COLORS[x]||'#6aa9ff');merged.partyColors[merged.governmentParty]=merged.partyProfile?.color||merged.partyColors[merged.governmentParty]||'#3b82f6';}
    return merged;
  }
  function loadState(){
    try{
      ...
      if(!Object.keys(slots).length){
        ...
        if(legacy){
          const id=makeSlotId();const migrated=hydrateState(JSON.parse(legacy));migrated.saveSlotId=id;
          slots[id]={state:migrated,updatedAt:new Date().toISOString()};writeSlots(slots);activeSlotId=id;localStorage.setItem(ACTIVE_SLOT_KEY,id);return migrated;
        }
      }
      if(activeSlotId&&slots[activeSlotId]?.state)return hydrateState(slots[activeSlotId].state);
      const first=Object.keys(slots)[0];
      if(first){activeSlotId=first;localStorage.setItem(ACTIVE_SLOT_KEY,first);return hydrateState(slots[first].state);}
      const id=makeSlotId();const fresh=initialState();fresh.saveSlotId=id;activeSlotId=id;localStorage.setItem(ACTIVE_SLOT_KEY,id);slots[id]={state:fresh,updatedAt:new Date().toISOString()};writeSlots(slots);return fresh;
    }catch{return initialState();}
  }

  function syncPartyRegistry(){
    const player=state.governmentParty||'Progressive Labour';
    if(!state.partyColors)state.partyColors={...BASE_PARTY_COLORS};
    state.partyColors[player]=state.partyProfile?.color||state.partyColors[player]||BASE_PARTY_COLORS[player]||'#3b82f6';
    const known=[...new Set([...Object.keys(state.polls||{}),...Object.keys(state.seats||{}),...Object.keys(state.partyColors||{})])];
    const active=known.filter(p=>(state.polls&&p in state.polls)||(state.seats&&p in state.seats));
    partyOrder=active.length?active.slice(0,6):Object.keys(BASE_PARTY_COLORS);
    if(!partyOrder.includes(player))partyOrder=[player,...partyOrder.filter(x=>x!==player)].slice(0,6);
    PARTY_COLORS={};partyOrder.forEach(p=>PARTY_COLORS[p]=state.partyColors[p]||BASE_PARTY_COLORS[p]||'#8194aa');
  }

  function slotMeta(st){return {leader:st.primeMinister||'New leader',party:st.governmentParty||'New party',phase:st.phase,date:st.date,history:(st.premiershipHistory||[]).length,ended:!!st.constitutional?.governmentEnded};}
  function saveLocal(show=true){
    if(!activeSlotId){activeSlotId=state.saveSlotId||makeSlotId();localStorage.setItem(ACTIVE_SLOT_KEY,activeSlotId);}
    state.saveSlotId=activeSlotId;const slots=readSlots();slots[activeSlotId]={state,meta:slotMeta(state),updatedAt:new Date().toISOString()};writeSlots(slots);
    $('#saveText').textContent='Premiership saved locally';
    if(show)toast('Premiership saved');
  }
  ...
  async function loadCloud(uid){
    if(!supabase) return;
    const {data,error}=await supabase.from('games').select('id,state').eq('user_id',uid).eq('slot_name',`MANDATE v5 ${activeSlotId}`).maybeSingle();
    if(error) return;
    if(data?.state){ cloudGameId=data.id; state=hydrateState(data.state);state.saveSlotId=activeSlotId;syncPartyRegistry();saveLocal(false);render(); }
  }
  ...
  async function saveCloud(){
    ...
    try{
      ...
      if(!user) return;
      const payload={user_id:user.id,slot_name:`MANDATE v5 ${activeSlotId}`,country_name:state.country,current_week:Math.max(1,state.week),state,updated_at:new Date().toISOString()};
      let res;
      if(cloudGameId) res=await supabase.from('games').update(payload).eq('id',cloudGameId).select().single();
      else res=await supabase.from('games').upsert(payload,{onConflict:'user_id,slot_name'}).select().single();
  ...
  function likenessScore(){return clamp(state.approval-50,-50,50);}
  function partyLeader(p){return state.partyLeaders?.[p]||`${p.split(' ')[0]} Leader`; }
  function electionWinner(seats=state.seats){return Object.keys(seats).sort((a,b)=>(seats[b]||0)-(seats[a]||0))[0];}
  function voteKey(type,date=state.date){return `${type}:${date}`;}
  function hasVoted(type,date=state.date){return !!state.personalVotes?.[voteKey(type,date)];}
  function archivePremiership(reason){
    if(!state.premiershipStartedDate)return;
    const already=(state.premiershipHistory||[]).some(x=>x.startDate===state.premiershipStartedDate&&x.endDate===state.date);if(already)return;
    const since=(state.history||[]).filter(x=>!x.date||x.date>=state.premiershipStartedDate);const electionsSince=(state.electionHistory||[]).filter(e=>e.date>=state.premiershipStartedDate&&e.governmentParty===state.governmentParty&&e.won);
    state.premiershipHistory.push({name:state.primeMinister,party:state.governmentParty,startDate:state.premiershipStartedDate,endDate:state.date,endReason:reason,weeks:state.week,finalLikeness:likenessScore(),peakPopularity:Math.max(state.approval,...since.map(x=>x.approval||0)),actsPassed:(state.bills||[]).filter(b=>b.stage>=5).length,electionsWon:electionsSince.length});
    state.premiershipStartedDate=null;
  }
  function firstThursdayOfMay(year){
    const d=new Date(Date.UTC(year,4,1));const add=(4-d.getUTCDay()+7)%7;d.setUTCDate(1+add);return d.toISOString().slice(0,10);
  }
  function nextLocalElectionAfter(iso){
    const y=+iso.slice(0,4);const thisYear=firstThursdayOfMay(y);return iso<thisYear?thisYear:firstThursdayOfMay(y+1);
  }
  function dateCrossed(from,to,target){return !!target&&from<=target&&to>=target;}
  function ensureLocalElection(){
    state.localElections=state.localElections||{nextDate:null,current:null,history:[]};
    if(!state.localElections.nextDate)state.localElections.nextDate=nextLocalElectionAfter(state.date);
    if(reached(state.localElections.nextDate)&&!state.localElections.current){
      state.localElections.current={date:state.localElections.nextDate,result:null};
    }
  }
  function localElectionDue(){ensureLocalElection();return !!state.localElections.current&&!state.localElections.current.result;}
  function runLocalElection(chosenParty){
    ensureLocalElection();const le=state.localElections.current;if(!le||le.result)return;
    if(chosenParty)state.personalVotes[voteKey('local',le.date)]={party:chosenParty,date:le.date,type:'local'};
    const weights={};let sum=0;partyOrder.forEach(p=>{const voterBoost=chosenParty===p?.15:0;weights[p]=Math.max(.5,(state.polls[p]||1)+r(-2.4,2.4)+voterBoost);sum+=weights[p];});
    const councilSeats={};let allocated=0;partyOrder.forEach((p,i)=>{const n=i===partyOrder.length-1?3000-allocated:Math.max(0,Math.round(weights[p]/sum*3000));councilSeats[p]=n;allocated+=n;});
    const winner=electionWinner(councilSeats);const govShare=councilSeats[state.governmentParty]||0;const benchmark=3000*((state.polls[state.governmentParty]||0)/100);const verdict=govShare>=benchmark?'better than expected':'worse than expected';
    le.result={winner,seats:councilSeats,personalVote:chosenParty||null,verdict};state.localElections.history.push({...le.result,date:le.date});state.localElections.nextDate=nextLocalElectionAfter(le.date);state.localElections.current=null;
    const swing=govShare>=benchmark?r(.2,1.1):r(-1.4,-.2);state.approval=clamp(state.approval+swing,0,100);
    addNews('Election',`${winner} tops the local elections`,`${state.governmentParty} performs ${verdict}. The Prime Minister's likeness moves ${signed(swing,'%')}.`);saveLocal(false);render();
    openModal(`<div class="eyebrow">LOCAL ELECTION RESULTS</div><h2>${escapeHtml(winner)} finishes first</h2><p class="muted">3,000 council seats have been declared across Avalon.</p><div class="seat-summary">${partyOrder.map(p=>`<div class="party-row"><div><i style="background:${PARTY_COLORS[p]}"></i><b>${escapeHtml(p)}</b></div><span>${councilSeats[p]}</span></div>`).join('')}</div><div class="callout section-space">Your ballot: <b>${escapeHtml(chosenParty||'Abstained')}</b></div>`);
  }
  function openLocalBallot(){
    ensureLocalElection();const le=state.localElections.current;if(!le)return;
    openModal(`<div class="eyebrow">LOCAL ELECTIONS • ${prettyDate(le.date)}</div><h2>Vote in your local council election</h2><p class="muted">Choose any national party. Your personal ballot is one vote, while the national council result is driven mainly by public polling and local variation.</p><div class="ballot-paper section-space">${partyOrder.map(p=>`<button class="ballot-choice" data-local-vote="${escapeHtml(p)}"><i style="background:${PARTY_COLORS[p]}"></i><span><b>${escapeHtml(p)}</b><small>${escapeHtml(partyLeader(p))}</small></span><strong>□</strong></button>`).join('')}</div><button class="ghost full section-space" id="abstainLocalVote">Abstain</button>`);
    document.querySelectorAll('[data-local-vote]').forEach(b=>b.onclick=()=>{closeModal();runLocalElection(b.dataset.localVote)});$('#abstainLocalVote').onclick=()=>{closeModal();runLocalElection(null)};
  }
  function createNewPremiershipSlot(){
    saveLocal(false);const slots=readSlots();const id=makeSlotId();const fresh=initialState();fresh.saveSlotId=id;slots[id]={state:fresh,meta:slotMeta(fresh),updatedAt:new Date().toISOString()};writeSlots(slots);activeSlotId=id;localStorage.setItem(ACTIVE_SLOT_KEY,id);state=hydrateState(fresh);cloudGameId=null;currentView='dashboard';syncPartyRegistry();saveLocal(false);closeModal();render();toast('New premiership created');
  }
  function switchPremiershipSlot(id){
    const slots=readSlots();if(!slots[id]?.state)return;saveLocal(false);activeSlotId=id;localStorage.setItem(ACTIVE_SLOT_KEY,id);state=hydrateState(slots[id].state);cloudGameId=null;currentView='dashboard';syncPartyRegistry();closeModal();render();toast('Premiership loaded');
  }
  function openCareerHub(){
    saveLocal(false);const slots=readSlots();const entries=Object.entries(slots).sort((a,b)=>String(b[1].updatedAt||'').localeCompare(String(a[1].updatedAt||'')));
    openModal(`<div class="eyebrow">MANDATE CAREERS</div><h2>Your premierships</h2><p class="muted">Each slot is a separate political career. Starting another one does not delete the government you are currently playing.</p><div class="career-list section-space">${entries.map(([id,x],i)=>{const st=x.state||{};const m=x.meta||slotMeta(st);const current=id===activeSlotId;return `<div class="career-card ${current?'current':''}"><div><span class="tag ${current?'blue-tag':''}">${current?'CURRENT':'SAVE '+(i+1)}</span><h3>${escapeHtml(m.leader||'New leader')}</h3><p>${escapeHtml(m.party||'Party not created')} • ${prettyDate(m.date||'2029-05-01')}</p><small>${m.phase==='setup'?'Not yet campaigned':`${(st.premiershipHistory||[]).length} completed premiership${(st.premiershipHistory||[]).length===1?'':'s'} • ${String(m.phase||'').replaceAll('_',' ')}`}</small></div>${current?'<strong>Playing</strong>':`<button class="secondary" data-load-slot="${id}">Continue</button>`}</div>`}).join('')}</div><button class="primary full section-space" id="createPremiershipSlotBtn">＋ START NEW PREMIERSHIP</button>`);
    document.querySelectorAll('[data-load-slot]').forEach(b=>b.onclick=()=>switchPremiershipSlot(b.dataset.loadSlot));$('#createPremiershipSlotBtn').onclick=createNewPremiershipSlot;
  }
  ...
  function openPersonalBallot(type,date=state.date,afterCast=null){
    const existing=state.personalVotes?.[voteKey(type,date)];
    openModal(`<div class="eyebrow">SECRET BALLOT • ${escapeHtml(type.toUpperCase())}</div><h2>Cast your personal vote</h2><p class="muted">You are still one voter. You can vote for your own party, a rival, or whoever you genuinely want. Your choice is recorded in this career.</p>${existing?`<div class="callout section-space">Already voted: <b>${escapeHtml(existing.party)}</b></div>`:''}<div class="ballot-paper section-space">${partyOrder.map(p=>`<button class="ballot-choice" data-personal-vote="${escapeHtml(p)}"><i style="background:${PARTY_COLORS[p]}"></i><span><b>${escapeHtml(p)}</b><small>Leader: ${escapeHtml(partyLeader(p))}</small></span><strong>□</strong></button>`).join('')}</div><div class="small muted section-space">Your ballot is private inside the simulation, but saved in your election history.</div>`);
    document.querySelectorAll('[data-personal-vote]').forEach(b=>b.onclick=()=>{state.personalVotes[voteKey(type,date)]={party:b.dataset.personalVote,date,type};saveLocal(false);closeModal();toast(`Ballot cast for ${b.dataset.personalVote}`);if(afterCast)afterCast();else render();});
  }
  ...
  function takeoverElectionWinner(){
    const result=state.phase==='defeat'?(state.openingCampaign?.result||{}):(state.postElectionChoice||{});
    const winner=result.winningParty||electionWinner();const leader=result.winningLeader||partyLeader(winner);const seats=result.winningSeats||state.seats[winner]||0;
    state.governmentParty=winner;state.primeMinister=leader;state.identityLocked=true;state.identityOrigin='election_successor';state.partyLeaders[winner]=leader;
    const initials=winner.split(/\s+/).map(x=>x[0]).join('').slice(0,5).toUpperCase();state.partyProfile={shortName:initials,slogan:'A new government begins',color:state.partyColors[winner]||BASE_PARTY_COLORS[winner]||'#3b82f6',ideology:'Inherited party programme'};
    state.majority=Math.max(0,seats-321);state.nextElectionWeeks=208;state.openingCampaign.result={playerSeats:seats,majority:state.majority,won:true,winningParty:winner,winningSeats:seats,winningLeader:leader,date:state.date};state.phase='cabinet_setup';state.postElectionChoice=null;ensureConstitutional();state.constitutional.governmentEnded=false;syncPartyRegistry();saveLocal(false);render();toast(`You now control ${leader}`);
  }
  function createLeadershipContest(){
    const pool=[...state.cabinet].sort((a,b)=>(b.competence+b.loyalty)-(a.competence+a.loyalty));
    if(pool.length<2){state.phase='career_ended';return;}
    const a=pool[0],b=pool[Math.min(pool.length-1,1+Math.floor(Math.random()*Math.min(3,pool.length-1)))];
    state.leadershipContest={date:state.date,candidates:[{id:a.id,name:a.name,role:a.role,competence:a.competence,loyalty:a.loyalty},{id:b.id,name:b.name,role:b.role,competence:b.competence,loyalty:b.loyalty}],personalVote:null,winner:null,scores:null};state.phase='leadership_contest';
  }
  function renderLeadershipContestScreen(){
    const c=state.leadershipContest;if(!c)return `<div class="panel"><h2>No leadership contest is active.</h2></div>`;
    if(!c.winner)return `<div class="campaign-shell"><div class="campaign-hero blue"><div><span class="phase-chip">PARTY LEADERSHIP ELECTION</span><h1>Your party chooses its next leader.</h1><p>Your resignation is complete. Two senior Cabinet ministers have reached the final ballot. You get a vote too.</p></div><div class="campaign-big-number">2<small>final candidates</small></div></div><div class="leadership-grid section-space">${c.candidates.map(x=>`<button class="leadership-candidate" data-leadership-vote="${x.id}"><span class="minister-avatar">${escapeHtml(x.name.split(/\s+/).map(n=>n[0]).join('').slice(0,2))}</span><div><small>${escapeHtml(x.role)}</small><h2>${escapeHtml(x.name)}</h2><p>Competence ${x.competence}% • Cabinet loyalty ${x.loyalty}%</p></div><strong>VOTE →</strong></button>`).join('')}</div><div class="callout section-space">This is a secret party ballot. You cannot rename either candidate.</div></div>`;
    const w=c.winner;return `<div class="campaign-shell"><div class="election-night-card"><span class="phase-chip">LEADERSHIP RESULT</span><div class="ballot-icon">✓</div><h1>${escapeHtml(w.name)} wins the leadership.</h1><p>The governing party has chosen its new leader following your resignation.</p><div class="election-preview"><span>New party leader</span><b>${escapeHtml(w.name)}</b><small>${escapeHtml(w.role)} • your vote: ${escapeHtml(c.personalVoteName||'Recorded')}</small></div><div class="choice-stack"><button class="primary big-primary" id="continueAsLeadershipWinnerBtn">Continue as ${escapeHtml(w.name)} →</button><button class="secondary big-primary" id="endCareerAfterLeadershipBtn">End my career here</button></div><div class="callout section-space"><b>Identity locked:</b> continuing means your controlled Prime Minister is now ${escapeHtml(w.name)}. The name cannot be changed.</div></div></div>`;
  }
  function castLeadershipVote(id){
    const c=state.leadershipContest;if(!c||c.winner)return;const chosen=c.candidates.find(x=>x.id==id);if(!chosen)return;c.personalVote=chosen.id;c.personalVoteName=chosen.name;
    const scores={};c.candidates.forEach(x=>scores[x.id]=x.competence*.46+x.loyalty*.34+r(0,24)+(x.id==chosen.id?6:0));const winner=[...c.candidates].sort((a,b)=>scores[b.id]-scores[a.id])[0];c.scores=scores;c.winner=winner;
    addNews('Government',`${winner.name} wins the leadership contest`,`${winner.name} is elected leader of ${state.governmentParty} after the outgoing Prime Minister's resignation.`);saveLocal(false);render();
  }
  function continueAsLeadershipWinner(){
    const c=state.leadershipContest;if(!c?.winner)return;const w=c.winner;const old=state.cabinet.find(x=>x.id==w.id);const role=old?.role||'Minister without Portfolio';const replacementNames=['Harriet Cole','Nathan Brooks','Leila Warren','George Mercer','Freya Collins','Isaac Rowan','Maya Sterling','Benjamin Hale'];const replacement=replacementNames[Math.floor(Math.random()*replacementNames.length)];
    state.cabinet=state.cabinet.map(m=>m.id==w.id?{...m,name:replacement,profile:replacement.split(/\s+/).map(x=>x[0]).join('').slice(0,2),competence:Math.round(r(58,87)),loyalty:Math.round(r(62,90))}:m);
    state.primeMinister=w.name;state.partyLeaders[state.governmentParty]=w.name;state.identityLocked=true;state.identityOrigin='cabinet_successor';state.phase='government';state.week=1;state.premiershipStartedDate=state.date;state.approval=clamp(48+r(-3,5),35,60);state.politicalCapital=68;state.pmqsUsed=false;state.nextPMQsWeek=2;state.cabinetMeetingUsed=false;state.weeklyEvent=null;state.leadershipContest=null;state.leadershipPressure={active:false,weeks:0,score:0,lastWeek:0,calls:[]};ensureConstitutional();state.constitutional.governmentEnded=false;state.constitutional.resignation=null;state.constitutional.snapElection=null;ensureWeeklyEvent();
    addNews('Government',`${w.name} begins a new premiership`,`${w.name} enters office after winning the governing party leadership contest. ${replacement} joins Cabinet as ${role}.`);saveLocal(false);currentView='dashboard';render();toast('New premiership begins');
  }
  function endCareer(reason='Career ended'){
    if(state.premiershipStartedDate)archivePremiership(reason);state.phase='career_ended';ensureConstitutional();state.constitutional.governmentEnded=true;saveLocal(false);render();
  }

  function setupPollsForPlayer(){
    const player=state.governmentParty;
    const rivals=Object.keys(BASE_PARTY_COLORS).filter(p=>p!==player&&p!=='Progressive Labour').slice(0,5);
    partyOrder=[player,...rivals];PARTY_COLORS={};
    state.partyColors=state.partyColors||{};PARTY_COLORS[player]=state.partyProfile.color;state.partyColors[player]=state.partyProfile.color;
    rivals.forEach(p=>{PARTY_COLORS[p]=BASE_PARTY_COLORS[p];state.partyColors[p]=BASE_PARTY_COLORS[p];});
    const vals=[31.5,14.0,9.0,6.5,4.8];
    state.polls={[player]:34.2};rivals.forEach((p,i)=>state.polls[p]=vals[i]||2);normalizePolls();
  ...
  function renderPregame(){
    ...
    if(state.phase==='election'){
      const voted=hasVoted('general',state.date);const ballot=state.personalVotes?.[voteKey('general',state.date)];
      return `<div class="campaign-shell"><div class="election-night-card"><span class="phase-chip">GENERAL ELECTION • POLLING DAY</span><div class="ballot-icon">✓</div><h1>It is polling day.</h1><p>Before the national count, you can cast your own ballot for <b>any party</b>. Then the 640 constituency results will be declared.</p><div class="election-preview"><span>${escapeHtml(state.governmentParty)}</span><b>${fmt(state.polls[state.governmentParty],1)}%</b><small>final campaign poll • score ${fmt(state.openingCampaign.score||0,1)}</small></div>${voted?`<div class="callout">Your vote: <b>${escapeHtml(ballot.party)}</b> ✓</div>`:'<button class="secondary big-primary" id="openingVoteBtn">CAST YOUR VOTE →</button>'}<button class="primary big-primary section-space" id="countElectionBtn" ${voted?'':'disabled'}>Begin election night →</button></div></div>`;
    }
    if(state.phase==='defeat'){
      const result=state.openingCampaign?.result||{};const wp=result.winningParty||electionWinner();const wl=result.winningLeader||partyLeader(wp);
      return `<div class="campaign-shell"><div class="election-night-card defeat-card"><span class="phase-chip">GENERAL ELECTION • FINAL RESULT</span><div class="ballot-icon defeat">✕</div><h1>The country chose another government.</h1><p>${escapeHtml(state.primeMinister)} fought the campaign, but <b>${escapeHtml(state.governmentParty)}</b> finished on <b>${result.playerSeats||0} seats</b>. <b>${escapeHtml(wp)}</b> emerged as the winning party.</p><div class="election-preview"><span>${escapeHtml(wp)}</span><b>${result.winningSeats||state.seats[wp]||0} seats</b><small>Incoming leader: ${escapeHtml(wl)}</small></div><div class="choice-stack"><button class="primary big-primary" id="takeOverElectionWinnerBtn">Continue career as ${escapeHtml(wl)} →</button><button class="secondary big-primary" id="newPremiershipAfterDefeatBtn">Start a separate new premiership</button></div><div class="callout section-space"><b>Identity rule:</b> if you take over the winning party, your name becomes ${escapeHtml(wl)} and cannot be edited.</div></div></div>`;
    }
    if(state.phase==='election_defeat'){
      const x=state.postElectionChoice||{};
      return `<div class="campaign-shell"><div class="election-night-card defeat-card"><span class="phase-chip">GENERAL ELECTION • CHANGE OF GOVERNMENT</span><div class="ballot-icon defeat">↻</div><h1>Your premiership has ended at the ballot box.</h1><p>${escapeHtml(x.oldLeader||state.primeMinister)} has lost office. <b>${escapeHtml(x.winningParty||'The opposition')}</b> will form the next administration.</p><div class="election-preview"><span>Incoming Prime Minister</span><b>${escapeHtml(x.winningLeader||'New leader')}</b><small>${x.winningSeats||0} seats</small></div><div class="choice-stack"><button class="primary big-primary" id="takeOverElectionWinnerBtn">Become ${escapeHtml(x.winningLeader||'the new Prime Minister')} →</button><button class="secondary big-primary" id="returnCareerHubBtn">Finish this career here</button></div><div class="callout section-space">Continuing changes the politician you control. The incoming Prime Minister's name is fixed.</div></div></div>`;
    }
    if(state.phase==='leadership_contest')return renderLeadershipContestScreen();
    if(state.phase==='career_ended'){
      const history=(state.premiershipHistory||[]).slice().reverse();return `<div class="campaign-shell"><div class="campaign-hero blue"><div><span class="phase-chip">POLITICAL CAREER ARCHIVE</span><h1>This career has reached its final page.</h1><p>Your completed premierships remain saved. You can reopen another save or start an entirely separate political career.</p></div><div class="campaign-big-number">${history.length}<small>premiership${history.length===1?'':'s'} recorded</small></div></div><div class="panel section-space"><div class="premiership-history">${history.length?history.map(x=>`<div class="history-premier"><div><b>${escapeHtml(x.name)}</b><span>${escapeHtml(x.party)} • ${prettyDate(x.startDate)} – ${prettyDate(x.endDate)}</span></div><strong>${signed(x.finalLikeness,'%')}</strong><small>${escapeHtml(x.endReason)}</small></div>`).join(''):'<div class="empty">No completed premierships were recorded.</div>'}</div><button class="primary full section-space" id="careerHubFromEndedBtn">OPEN PREMIERSHIP SAVES</button></div></div>`;
    }
  ...
  function beginOpeningCampaign(){
    ...
    state.primeMinister=leader;state.governmentParty=party;state.identityLocked=false;state.identityOrigin='created';
    state.partyProfile={shortName,slogan:slogan||'A new chapter for Avalon',color:$('#setupColor').value,ideology:$('#setupIdeology').value};
    state.partyColors={...BASE_PARTY_COLORS};if(party!=='Progressive Labour')delete state.partyColors['Progressive Labour'];state.partyColors[party]=state.partyProfile.color;
    state.partyLeaders={...initialState().partyLeaders,[party]:leader};if(party!=='Progressive Labour')delete state.partyLeaders['Progressive Labour'];
  ...
  function holdOpeningElection(){
    ...
    if(!hasVoted('general',state.date)){openPersonalBallot('general',state.date,render);toast('Cast your ballot before election night');return;}
    ...
    const playerSeats=newSeats[state.governmentParty];const won=playerSeats>=321;const winningParty=electionWinner(newSeats);const winningSeats=newSeats[winningParty];const winningLeader=partyLeader(winningParty);
    state.seats=newSeats;state.majority=won?playerSeats-321:0;state.electionHeld=true;
    state.openingCampaign.result={playerSeats,majority:state.majority,poll:playerPoll,effectivePoll:effectivePolls[state.governmentParty],date:state.date,won,winningParty,winningSeats,winningLeader};
    state.electionHistory.push({type:'general',date:state.date,governmentParty:state.governmentParty,seats:{...newSeats},won,winningParty,personalVote:state.personalVotes[voteKey('general',state.date)]?.party||null});
    if(won){
  ...
  function formOpeningCabinet(){
    ...
    state.phase='government';state.week=1;state.date=isoAddDays(state.date,1);state.premiershipStartedDate=state.date;state.pmqsUsed=false;state.nextPMQsWeek=2;state.cabinetMeetingUsed=false;state.pmqsHistory=[];state.cabinetMeetingHistory=[];
    if(!state.localElections.nextDate)state.localElections.nextDate=nextLocalElectionAfter(state.date);
    state.constitutional.governmentEnded=false;state.constitutional.resignation=null;state.constitutional.snapElection=null;state.leadershipPressure={active:false,weeks:0,score:0,lastWeek:0,calls:[]};
  ...
  function updateChrome(){
    ...
    $('#approvalTop').textContent=pollingPhase?`${fmt(state.polls?.[state.governmentParty]||0,1)}%`:`${signed(likenessScore(),'%')}`;
    const approvalLabel=document.querySelector('.approval-pill small');if(approvalLabel)approvalLabel.textContent=pollingPhase?'Polling':'Likeness';
    ...
    else if(state.phase==='election_defeat'){next.classList.add('hidden');$('#viewEyebrow').textContent='CHANGE OF GOVERNMENT';$('#viewTitle').textContent='Election defeat';$('#brandSub').textContent='Avalon • Transfer of power';}
    else if(state.phase==='leadership_contest'){next.classList.add('hidden');$('#viewEyebrow').textContent='LEADERSHIP ELECTION';$('#viewTitle').textContent='Choose the next party leader';$('#brandSub').textContent=`${state.partyProfile.shortName} • Leadership contest`;}
    else if(state.phase==='career_ended'){next.classList.add('hidden');$('#viewEyebrow').textContent='CAREER ARCHIVE';$('#viewTitle').textContent='Premiership record';$('#brandSub').textContent='MANDATE • Political archive';}
  ...
  function renderDashboard(){
    ...
    return `
      ...
      <div class="popularity-panel section-space">
        <div class="panel-title-row"><div><div class="eyebrow">PRIME MINISTER LIKENESS</div><h3>Net public feeling toward ${escapeHtml(state.primeMinister)}</h3></div><strong class="popularity-number">${signed(likenessScore(),'%')}</strong></div>
        ...
        <div class="popularity-labels"><span>-50 hated</span><span>0 divided</span><span>+50 adored</span></div>
        <div class="small muted section-space">Cabinet resignation pressure begins at <b>-20%</b> likeness.</div>
      </div>
      ${renderLeadershipPressureCard()}
  ...
  function renderLeadershipPressureCard(){
    const p=state.leadershipPressure||{};if(!p.active)return '';
    const latest=p.calls?.[p.calls.length-1];return `<div class="pressure-card section-space"><div><span class="phase-chip danger-chip">LEADERSHIP PRESSURE</span><h3>Cabinet is questioning whether you can continue.</h3><p>Your likeness is ${signed(likenessScore(),'%')}. Ministers, MPs and party figures are increasingly discussing resignation.</p>${latest?`<small>Latest: ${escapeHtml(latest.name)} • ${escapeHtml(latest.message)}</small>`:''}</div><div class="pressure-score"><b>${Math.round(p.score||0)}%</b><span>pressure</span></div><button class="danger-button" data-jump="transitions">Open resignation options</button></div>`;
  }
  ...
  function renderCalendar(){
    ensureWeeklyEvent();ensureLocalElection();
    ...
    for(let n=0;n<42;n++){
      ...
      if(date===(state.localElections.current?.date||state.localElections.nextDate))items.push({type:'local',label:'Local elections'});
  ...
  function renderElections(){
    ensureLocalElection();const generalDue=state.nextElectionWeeks<=0;const votedGeneral=hasVoted('general',state.date);const local=state.localElections.current;const latestLocal=state.localElections.history?.[state.localElections.history.length-1];
    return `<div class="grid grid-2"><div class="panel"><div class="panel-title-row"><h3>National voting intention</h3><span class="tag">Election in ${Math.max(0,state.nextElectionWeeks)} weeks</span></div><div class="poll-chart">${partyOrder.map(p=>`<div class="poll-row"><label>${escapeHtml(p)}</label><div class="poll-track"><span style="width:${state.polls[p]*2.2}%;background:${PARTY_COLORS[p]}"></span></div><b>${fmt(state.polls[p],1)}%</b></div>`).join('')}</div><div class="divider"></div><button class="secondary" id="commissionPoll">Commission private poll • £0.4bn</button>${generalDue?`<div class="polling-booth section-space"><div><span class="phase-chip">GENERAL ELECTION • POLLING DAY</span><h3>Your ballot</h3><p>${votedGeneral?`You voted for <b>${escapeHtml(state.personalVotes[voteKey('general',state.date)].party)}</b>.`:'Cast your personal vote before the national result is declared.'}</p></div>${votedGeneral?'<span class="vote-tick">✓</span>':'<button class="secondary" id="generalVoteBtn">VOTE</button>'}</div>`:''}</div><div class="panel"><h3>Campaign operations</h3><p class="small muted">Spend campaign funds for a short-term polling lift. Elections can remove you from office.</p><div class="campaign-actions"><button class="campaign-action" data-campaign="rally"><b>National rally</b><span>Cost £1.2bn • energise base</span></button><button class="campaign-action" data-campaign="broadcast"><b>Prime-time broadcast</b><span>Cost £0.8bn • reach swing voters</span></button><button class="campaign-action" data-campaign="ground"><b>Ground campaign</b><span>Cost £1.6bn • target marginal seats</span></button></div><div class="divider"></div><div class="flex between small"><span class="muted">Campaign fund</span><b>${money(state.campaignFunds)}</b></div>${generalDue?`<button class="primary full section-space" id="holdElection" ${votedGeneral?'':'disabled'}>DECLARE GENERAL ELECTION RESULT</button>`:''}</div></div>
    <div class="panel section-space local-election-panel"><div class="panel-title-row"><div><div class="eyebrow">LOCAL DEMOCRACY</div><h3>${local?'Local election polling day':'Next local elections'}</h3></div><span class="tag ${local?'gold':'blue-tag'}">${prettyDate(local?.date||state.localElections.nextDate)}</span></div>${local?`<p class="muted small">Council seats across Avalon are being contested today. You may vote for any party.</p><button class="primary" id="localVoteBtn">CAST LOCAL ELECTION BALLOT →</button>`:`<p class="muted small">Local elections take place every year on the first Thursday in May. They do not directly remove the Prime Minister, but a bad result can damage your likeness and trigger party pressure.</p>`}${latestLocal?`<div class="callout section-space">Last result: <b>${escapeHtml(latestLocal.winner)}</b> finished first. Your vote: ${escapeHtml(latestLocal.personalVote||'Abstained')}.</div>`:''}</div>`;
  }
  ...
  function holdElection(type='general',ballotDate=state.date){
    if(!hasVoted(type,ballotDate)){openPersonalBallot(type,ballotDate,render);toast('Cast your ballot before the result');return false;}
    const raw={};let sum=0;partyOrder.forEach(p=>{const effective=Math.max(.5,(state.polls[p]||1)+r(-1.5,1.5));raw[p]=Math.max(1,effective**2.15);sum+=raw[p]});let allocated=0;const newSeats={};partyOrder.forEach((p,i)=>{newSeats[p]=i===partyOrder.length-1?640-allocated:Math.round(raw[p]/sum*640);allocated+=newSeats[p]});
    state.seats=newSeats;const oldParty=state.governmentParty,oldLeader=state.primeMinister;const govSeats=newSeats[oldParty]||0;const winningParty=electionWinner(newSeats),winningSeats=newSeats[winningParty]||0,winningLeader=partyLeader(winningParty);const won=winningParty===oldParty;
    state.electionHeld=true;state.nextElectionWeeks=208;state.majority=won?Math.max(0,govSeats-321):0;state.approval=clamp(state.approval+r(-2,2),0,100);state.electionHistory.push({type,date:state.date,governmentParty:oldParty,seats:{...newSeats},won,winningParty,personalVote:state.personalVotes[voteKey(type,ballotDate)]?.party||null});
    if(won){addNews('Election',`${oldParty} wins the general election`,`The new Parliament returns ${govSeats} seats for the governing party.`);saveLocal(false);render();openModal(`<div class="eyebrow">GENERAL ELECTION RESULT</div><h2>Government returned</h2><p class="muted">${escapeHtml(oldParty)}: <b>${govSeats}</b> seats out of 640.</p><div class="seat-summary">${partyOrder.map(p=>`<div class="party-row"><div><i style="background:${PARTY_COLORS[p]}"></i><b>${escapeHtml(p)}</b></div><span>${newSeats[p]}</span></div>`).join('')}</div><div class="callout section-space">Your vote: <b>${escapeHtml(state.personalVotes[voteKey(type,ballotDate)]?.party||'Recorded')}</b></div>`);return true;}
    archivePremiership('General election defeat');state.postElectionChoice={oldLeader,oldParty,winningParty,winningSeats,winningLeader,date:state.date,seats:{...newSeats}};state.phase='election_defeat';ensureConstitutional();state.constitutional.governmentEnded=true;addNews('Election',`${winningParty} defeats the government`,`${oldLeader} leaves office after ${winningParty} wins ${winningSeats} seats. ${winningLeader} is poised to become Prime Minister.`);saveLocal(false);render();return true;
  }
  ...
  function renderTransitions(){
    ...
    return `
      ...
      <div class="grid grid-2 section-space">
        ...
        <div class="panel constitutional-card"><div class="panel-title-row"><div><div class="eyebrow">RESIGNATION</div><h3>Leave office formally</h3></div><span class="tag ${res?'red':''}">${res?res.status.replaceAll('_',' '):'Not started'}</span></div><p class="small muted">Required route: announce resignation → make timetable → announce timetable → final-day royal audience and resignation. After the final audience, two Cabinet ministers contest the party leadership.</p>${res?resTimeline:`<button class="danger-button full section-space" id="startResignation" ${resLocked||c.governmentEnded?'disabled':''}>ANNOUNCE RESIGNATION</button>`}${res?resignationActionButton(res):''}${resLocked?'<div class="callout section-space">A snap-election process is already active.</div>':''}</div>
      </div>${state.leadershipPressure?.active?`<div class="pressure-card section-space"><div><span class="phase-chip danger-chip">PARTY REVOLT</span><h3>Pressure to resign: ${Math.round(state.leadershipPressure.score||0)}%</h3><p>Your net likeness is ${signed(likenessScore(),'%')}. Cabinet pressure began once it passed -20%.</p></div><div class="pressure-score"><b>${Math.round(state.leadershipPressure.score||0)}%</b><span>pressure</span></div></div>`:''}`;
  }
  ...
  function snapActionButton(snap){
    ...
    const voted=hasVoted('snap',snap.electionDate);return `<div class="polling-booth section-space"><div><b>Your polling-day ballot</b><span>${voted?`Voted for ${escapeHtml(state.personalVotes[voteKey('snap',snap.electionDate)].party)}`:'Choose any party before the count.'}</span></div>${voted?'<span class="vote-tick">✓</span>':'<button class="secondary" id="snapVoteBtn">VOTE</button>'}</div><button class="primary full section-space" id="snapPoll" ${voted?'':'disabled'}>HOLD GENERAL ELECTION</button>`;
  }
  ...
  function holdSnapElection(){const x=state.constitutional.snapElection;if(!x||!reached(x.electionDate))return;if(!hasVoted('snap',x.electionDate)){openPersonalBallot('snap',x.electionDate,render);return}const done=holdElection('snap',x.electionDate);if(done){x.completed=true;x.status='completed';saveLocal(false);}}
  ...
  function completeResignation(){
    const r=state.constitutional.resignation;if(!r||r.completed||!reached(r.finalDay))return;
    r.completed=true;r.status='completed';archivePremiership('Resignation');state.constitutional.governmentEnded=true;addNews('Government',`Prime Minister formally resigns to ${state.constitutional.monarch}`,`On ${prettyDate(state.date)}, ${state.primeMinister} met ${monarchLabel()} and formally resigned from office. A governing-party leadership contest now begins.`);createLeadershipContest();saveLocal(false);render();toast('Leadership contest begins');
  }
  ...
  function checkLeadershipPressure(){
    const score=likenessScore();
    const p=state.leadershipPressure||(state.leadershipPressure={active:false,weeks:0,score:0,lastWeek:0,calls:[]});
    if(score<=-20){
      p.active=true;
      if(p.lastWeek!==state.week){
        p.lastWeek=state.week;p.weeks=(p.weeks||0)+1;p.score=clamp((p.score||0)+10+Math.abs(score+20)*.8+r(0,5),0,100);
        const minister=[...state.cabinet].sort((a,b)=>a.loyalty-b.loyalty)[0];
        if(minister){
          minister.loyalty=clamp(minister.loyalty-r(2,6),0,100);
          const messages=['says the Government needs a change of direction','privately tells colleagues the Prime Minister should consider their position','warns that the party cannot ignore the polling','refuses to guarantee support for the Prime Minister'];
          const message=messages[Math.floor(Math.random()*messages.length)];
          p.calls.push({week:state.week,date:state.date,name:minister.name,message});if(p.calls.length>8)p.calls.shift();
          addNews('Government',`${minister.name} increases pressure on the Prime Minister`,`${minister.name} ${message}. Net public likeness is ${signed(score,'%')}.`);
        }
        state.politicalCapital=clamp(state.politicalCapital-r(1.5,3.5),0,100);state.cabinet.forEach(m=>m.loyalty=clamp(m.loyalty-r(.2,1.3),0,100));
      }
    }else if(score>-15&&p.active){
      p.score=clamp((p.score||0)-r(8,15),0,100);
      if(p.score<15){p.active=false;p.weeks=0;addNews('Government','Leadership pressure eases','Improving public sentiment has calmed immediate talk of resignation inside the governing party.');}
    }
  }
  ...
  function advanceWeek(){
    ...
    if(['defeat','election_defeat','leadership_contest','career_ended'].includes(state.phase)){toast('Resolve the current political transition first');return;}
    ...
    ensureLocalElection();if(localElectionDue()){openLocalBallot();toast('Local election polling day: vote or abstain first');return;}
    ensureWeeklyEvent();
    ...
    state.date=newDate;state.cabinetMeetingUsed=false;ensureLocalElection();
    ...
    movePoll(state.governmentParty,(state.approval-oldApproval)*.08+r(-.12,.12));checkLeadershipPressure();
  ...
  function bindViewEvents(){
    ...
    $('#openingVoteBtn')?.addEventListener('click',()=>openPersonalBallot('general',state.date,render));
    $('#countElectionBtn')?.addEventListener('click',holdOpeningElection);
    $('#formGovernmentBtn')?.addEventListener('click',formOpeningCabinet);
    $('#takeOverElectionWinnerBtn')?.addEventListener('click',takeoverElectionWinner);
    $('#newPremiershipAfterDefeatBtn')?.addEventListener('click',createNewPremiershipSlot);
    $('#returnCareerHubBtn')?.addEventListener('click',()=>{state.phase='career_ended';saveLocal(false);render();openCareerHub();});
    document.querySelectorAll('[data-leadership-vote]').forEach(b=>b.onclick=()=>castLeadershipVote(b.dataset.leadershipVote));
    $('#continueAsLeadershipWinnerBtn')?.addEventListener('click',continueAsLeadershipWinner);
    $('#endCareerAfterLeadershipBtn')?.addEventListener('click',()=>{state.leadershipContest=null;state.phase='career_ended';saveLocal(false);render();});
    $('#careerHubFromEndedBtn')?.addEventListener('click',openCareerHub);
    ...
    $('#generalVoteBtn')?.addEventListener('click',()=>openPersonalBallot('general',state.date,render));
    $('#localVoteBtn')?.addEventListener('click',openLocalBallot);
    $('#holdElection')?.addEventListener('click',()=>holdElection('general'));
    ...
    $('#snapVoteBtn')?.addEventListener('click',()=>{const x=state.constitutional?.snapElection;if(x)openPersonalBallot('snap',x.electionDate,render)});
  ...
  $('#careerHubBtn').onclick=openCareerHub;
  $('#resetBtn').onclick=()=>{openModal(`<div class="eyebrow">NEW PREMIERSHIP</div><h2>Start another political career?</h2><p class="muted">Your current premiership will stay saved. The new slot begins at party creation, followed by the two-week campaign and General Election.</p><div class="flex gap section-space"><button class="primary" id="confirmReset">Create new premiership</button><button class="secondary" id="cancelReset">Keep playing</button></div>`);$('#cancelReset').onclick=closeModal;$('#confirmReset').onclick=createNewPremiershipSlot;};
