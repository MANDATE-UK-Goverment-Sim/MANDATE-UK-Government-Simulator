...
-- v5 political-career systems -------------------------------------------------
create table if not exists public.premiership_history (
  id uuid primary key default gen_random_uuid(),
  game_id uuid not null references public.games(id) on delete cascade,
  ...
  end_date date,
  end_reason text,
  final_likeness numeric(6,2),
  summary jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

create table if not exists public.local_election_results (
  id uuid primary key default gen_random_uuid(),
  game_id uuid not null references public.games(id) on delete cascade,
  election_date date not null,
  winning_party text not null,
  council_seats jsonb not null default '{}'::jsonb,
  personal_vote text,
  created_at timestamptz not null default now(),
  unique(game_id,election_date)
);

create table if not exists public.leadership_contests (
  id uuid primary key default gen_random_uuid(),
  game_id uuid not null references public.games(id) on delete cascade,
...
create table if not exists public.personal_ballots (
  id uuid primary key default gen_random_uuid(),
  game_id uuid not null references public.games(id) on delete cascade,
  election_type text not null,
  election_date date not null,
  party_or_candidate text not null,
  created_at timestamptz not null default now(),
  unique(game_id,election_type,election_date)
);

create index if not exists idx_premiership_history_game on public.premiership_history(game_id,start_date desc);
create index if not exists idx_local_elections_game on public.local_election_results(game_id,election_date desc);
create index if not exists idx_leadership_contests_game on public.leadership_contests(game_id,contest_date desc);
create index if not exists idx_personal_ballots_game on public.personal_ballots(game_id,election_date desc);
...
alter table public.premiership_history enable row level security;
alter table public.local_election_results enable row level security;
alter table public.leadership_contests enable row level security;
alter table public.personal_ballots enable row level security;
...
drop policy if exists "parties through own game" on public.parties;
...
drop policy if exists "constituencies through own game" on public.constituencies;
create policy "constituencies through own game" on public.constituencies for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "bills through own game" on public.bills;
create policy "bills through own game" on public.bills for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "divisions through own game" on public.divisions;
create policy "divisions through own game" on public.divisions for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "cabinet through own game" on public.cabinet_members;
create policy "cabinet through own game" on public.cabinet_members for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "events through own game" on public.events;
create policy "events through own game" on public.events for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "polls through own game" on public.polling_snapshots;
create policy "polls through own game" on public.polling_snapshots for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "economy through own game" on public.economic_snapshots;
create policy "economy through own game" on public.economic_snapshots for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "news through own game" on public.news_items;
create policy "news through own game" on public.news_items for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));


drop policy if exists "constitutional events through own game" on public.constitutional_events;
create policy "constitutional events through own game" on public.constitutional_events for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));


-- v5 career-system RLS policies
drop policy if exists "premiership history through own game" on public.premiership_history;
create policy "premiership history through own game" on public.premiership_history for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "local elections through own game" on public.local_election_results;
create policy "local elections through own game" on public.local_election_results for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "leadership contests through own game" on public.leadership_contests;
create policy "leadership contests through own game" on public.leadership_contests for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));

drop policy if exists "personal ballots through own game" on public.personal_ballots;
create policy "personal ballots through own game" on public.personal_ballots for all
using (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()))
with check (exists(select 1 from public.games g where g.id=game_id and g.user_id=auth.uid()));
